...
pub struct Ready { pub revents: i16 }

pub fn poll_fds(fds: &[(RawFd, i16)], timeout_ms: i32) -> io::Result<Vec<(RawFd, Ready)>> {
    let mut pfds: Vec<libc::pollfd> = fds.iter().map(|(fd, ev)| libc::pollfd{ fd: *fd, events: *ev, revents: 0 }).collect();
    let rc = unsafe { libc::poll(pfds.as_mut_ptr(), (pfds.len() as u64).try_into().unwrap(), timeout_ms) };
    if rc < 0 { return Err(io::Error::last_os_error()); }
    let mut out = Vec::new();
    for p in pfds {
        if p.revents != 0 { out.push((p.fd, Ready{ revents: p.revents })); }
    }
    Ok(out)
}
