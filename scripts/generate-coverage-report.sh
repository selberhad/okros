...
grep -E "^[a-z_/]+\.rs " /tmp/coverage-summary.txt | grep -v "^TOTAL" | while IFS= read -r line; do
    ...
    if awk "BEGIN {exit !($LINE_COV < 40)}"; then
        ...
        REASON=""
        case "$MODULE" in
            "main.rs") REASON=" (event loop - needs integration tests)" ;;
            "tty.rs") REASON=" (requires real TTY)" ;;
            "curses.rs") REASON=" (ncurses FFI - TTY dependent)" ;;
        esac
        echo "- \`$MODULE\` - $(echo "$line" | awk '{print $10}')$REASON" >> "$TEMP_FILE"
    fi
done
